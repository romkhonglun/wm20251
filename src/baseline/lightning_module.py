...
class NAMLLightningModule(pl.LightningModule):
    def __init__(self,
        ...
        # 2. Load Embeddings (Gọi hàm đã tối ưu bộ nhớ)
        self._init_embeddings(embedding_dir)
    ...
    def _init_embeddings(self, embedding_dir):
        ...
        try:
            ...
            # Load bình thường (không mmap) để kiểm soát RAM chủ động
            val = np.load(f"{embedding_dir}/title_emb.npy")
            tensor = torch.tensor(val, dtype=torch.float32)
            del val  # Xóa ngay bản numpy khỏi RAM
            self.model.news_encoder.title_emb = nn.Embedding.from_pretrained(tensor, freeze=True)
            del tensor  # Xóa biến tạm
            gc.collect()  # Dọn dẹp RAM ngay lập tức

            # --- BODY ---
            print(" -> Loading Body Embedding...")
            val = np.load(f"{embedding_dir}/body_emb.npy")
            tensor = torch.tensor(val, dtype=torch.float32)
            del val
            self.model.news_encoder.body_emb = nn.Embedding.from_pretrained(tensor, freeze=True)
            del tensor
            gc.collect()

            # --- CAT ---
            print(" -> Loading Category Embedding...")
            val = np.load(f"{embedding_dir}/cat_emb.npy")
            tensor = torch.tensor(val, dtype=torch.float32)
            del val
            self.model.news_encoder.cat_emb = nn.Embedding.from_pretrained(tensor, freeze=True)
            del tensor
            gc.collect()
